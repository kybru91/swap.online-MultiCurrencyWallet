...
// Network-dependent tests broadcast real txs — require ACTIONS=true env var
const networkTest = process.env.ACTIONS === 'true' ? it : it.skip
...
describe('BTC Send Tests', () => {
  ...
  networkTest(
    'send and check transaction via regular wallet with 1000 satoshis',
    async () => {
  ...
  networkTest(
    'send and check transaction via regular wallet with 1000 satoshis with adminFee',
    async () => {
      const serviceFee = {
        fee: '5',
  ...
  networkTest(
    'send and check transaction via pin-protected wallet sign with mnemonic with 1000 satoshis',
    async () => {
  ...
  networkTest(
    'send and check transaction via pin-protected wallet sign with password with 1000 satoshis with adminFee',
    async () => {
      const serviceFee = {
        fee: '5',
        address: '2MuXz9BErMbWmoTshGgkjd7aMHeaxV8Bdkk',
      ...
      const txHash = await actions.btcmultisig.sendPinProtected(options)
      await timeOut(5 * 1000)
      const { amount, minerFee, adminFee, size } = (await fetchTxInfo(
        txHash.txId,
        true,
        serviceFee
      ))!
